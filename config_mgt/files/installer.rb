...
data_hash.each do |key,value|# hash...
  ...
  if(value['type'].eql?('service'))
    ...
    system "#{value['type']} #{value['name']} start > /dev/null" #if not clean uninstall
    if($?.exitstatus > 0)
      ...
      system "apt-get purge 'php5*' -y"
      system "sudo apt-get install -y php5 > /dev/null"
  ...
  if(value['type'].eql?('file'))
    ...
    if(read_file != ('<?php header("Content-Type: text/plain"); echo "Hello, world!\n";?>'))
      ...
      system "touch #{key} > /dev/null" #create the php
